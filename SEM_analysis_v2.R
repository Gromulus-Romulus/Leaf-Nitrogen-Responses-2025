...
# --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --  --
##' Now, we visualize the resulting path diagram of the best fit model
##' Code written by Nathan D. Malamud, Sean's grad student
##' 
...
# Extract the summary of the SEM model
sem_summary <- summary(mod)

# View the coefficients table from the summary
print(sem_summary$coefficients)


library(DiagrammeR)

# Create the summary data frame from the sem_summary$coefficients output
...
# Create a directed graph for the SEM
library(DiagrammeR)
...
grViz("
  digraph SEM {
    ...
    # Define causal paths with coefficients
    Nitrogen -> LMA [label = '-0.0023***', color = red]      # Negative relationship
    Nitrogen -> LDMC [label = '-0.0020**', color = red]      # Negative relationship
    Nitrogen -> CHL [label = '0.0061***', color = black]     # Positive relationship
    Nitrogen -> GrowthRate [label = '0.0119***', color = black] # Positive relationship
    LMA -> GrowthRate [label = '2.4784 (ns)', color = black] # Non-significant
    LDMC -> GrowthRate [label = '-2.4999***', color = red]   # Negative relationship
    CHL -> GrowthRate [label = '1.3530***', color = black]   # Positive relationship

    # Define correlations (bidirectional paths)
    LMA -> LDMC [dir=both, label = '0.7344***', color = grey]
  }
")
library(DiagrammeR)
