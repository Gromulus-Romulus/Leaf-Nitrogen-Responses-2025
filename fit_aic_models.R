...
#
log_traits <- traits %>%
  mutate(LMA = log(LMA + 1e-6),
         LDMC = log(LDMC + 1e-6),
         area_cm2 = log(area_cm2 + 1e-6),
         CHL = log(CHL + 1e-6),
         GRT = log(GRT + 1e-6),
         log_treatment_mmol = log(treatment_mmol + 1e-6)
         )  # Add small value to avoid log(0) errors

traits <- traits %>%
  mutate(log_treatment_mmol = log(treatment_mmol + 1e-6))
...
recip_traits <- traits %>%
  mutate(LMA = 1 / (LMA + 1e-6),
         LDMC = 1 / (LDMC + 1e-6),
         area_cm2 = 1 / (area_cm2 + 1e-6),
         CHL = 1 / (CHL + 1e-6),
         GRT = 1 / (GRT + 1e-6),
         recip_treatment_mmol = 1 / (treatment_mmol + 1e-6)
         )

traits <- traits %>%
  mutate(recip_treatment_mmol = 1 / (treatment_mmol + 1e-6))
...
recip_sqrt_traits <- traits %>%
  mutate(LMA = 1 / sqrt(LMA + 1e-6),
         LDMC = 1 / sqrt(LDMC + 1e-6),
         area_cm2 = 1 / sqrt(area_cm2 + 1e-6),
         CHL = 1 / sqrt(CHL + 1e-6),
         GRT = 1 / sqrt(GRT + 1e-6),
         recip_sqrt_treatment_mmol = 1 / sqrt(treatment_mmol + 1e-6)
         )

traits <- traits %>%
  mutate(recip_sqrt_treatment_mmol = 1 / sqrt(treatment_mmol + 1e-6))
...
aic_model_stats_filt.2 <- aic_model_stats_filt |>
  ...
  arrange(R2_conditional) |> filter(R2_conditional > 0.35) |>
